...
GROUP BY 
    ...
    t.login_name
	HAVING 
    DATEDIFF(SECOND, MAX(ses.last_request_start_time), GETDATE()) > 10
	order by duration desc;
    

    Select top 5 * from BlockedQueriesInfo where DATEDIFF(SECOND,logdate, GETDATE()) < 60 order by [WaitingInMinutes] desc
...
SET @subject = N'Test ' + @ServerName + ' - Blocking Queries -> Open'; --Change 1
...
(SELECT TOP (5)
    ...
    WHERE DATEDIFF(SECOND, b.logdate, GETDATE()) < 60
...
SELECT @Summary = (
    ...
    HAVING DATEDIFF(SECOND, MAX(ses.last_request_start_time), GETDATE()) > 10
    FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)');
...
@profile_name = 'DBA',     --Change 3 DBmail Profiler
@body = @HTML,
@body_format ='html',
@recipients = 'sivaraman@geopits.com',  -- change 4 Our mail or client mail
@copy_recipients = '',
...
DECLARE @HTML_Closed NVARCHAR(MAX) ,@Summary NVARCHAR(MAX), @ServerName NVARCHAR(128) = @@ServerName;
...
--Select @HTML_Closed;
 SET @subject = N'Test ' + @ServerName + ' - Blocking Queries -> Closed'; --Change 1
IF (@HTML_Closed IS NOT NULL)
BEGIN
EXEC msdb.dbo.sp_send_dbmail
@profile_name = 'th1',     -- Change 3 DBMail Profiler
@body = @HTML_Closed,
@body_format = 'html',
