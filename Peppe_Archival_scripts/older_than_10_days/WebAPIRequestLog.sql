...
Create or alter   PROCEDURE [dbo].[WebAPIRequestLog]
AS
BEGIN
    SET NOCOUNT ON;
    ...
    DECLARE @SourceTableName SYSNAME = '[DBLoanguard].[dbo].[WebAPIRequestLog]';
    DECLARE @DestinationTableName SYSNAME = '[DBLoanguardHistory].[dbo].[WebAPIRequestLog]';
    DECLARE @filter NVARCHAR(MAX) ='Date  < DATEADD(day, -10, CAST(GETDATE() AS date))'; 
	DECLARE @column NVARCHAR(MAX) ='Date';
...
;WITH CTE_ToDelete AS (
    SELECT TOP (' + CAST(@BatchSize AS NVARCHAR(10)) + N') 
       [LogId],[Date],[CorrelationId],[RequestMethod],[RequestHeader],[RequestUrl],[PostParameters],
	   [RequestOn],[UserId],[ResponseHeader],[ResponseContent],[ResponseOn]
...
OUTPUT 
	   ...
	   Deleted.[RequestMethod],
	   Deleted.[RequestHeader],
	   Deleted.[RequestUrl],
	   Deleted.[PostParameters],
	   Deleted.[RequestOn],
	   Deleted.[UserId],
	   Deleted.[ResponseHeader],
	   Deleted.[ResponseContent],
	   Deleted.[ResponseOn]  
