...
Create or alter   PROCEDURE [dbo].[AutoSamplingServiceLog]
AS
BEGIN
    SET NOCOUNT ON;
    ...
    DECLARE @SourceTableName SYSNAME = '[DBLoanguard].[dbo].[AutoSamplingServiceLog]';
    DECLARE @DestinationTableName SYSNAME = '[DBLoanguardHistory].[dbo].[AutoSamplingServiceLog]';
...
;WITH CTE_ToDelete AS (
    SELECT TOP (' + CAST(@BatchSize AS NVARCHAR(10)) + N') 
        [LogId],[LogDate],[Thread],[Level],[Logger],[Message],[Exception],
		[ApplicationNo],[LoanguardId],[DocumentId],[DocumentIndex],[ProductId]
