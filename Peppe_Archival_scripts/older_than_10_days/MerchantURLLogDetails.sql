...
Create or alter   PROCEDURE [dbo].[MerchantURLLogDetails]
AS
BEGIN
    SET NOCOUNT ON;
    ...
    DECLARE @SourceTableName SYSNAME = '[DBLoanguard].[dbo].[MerchantURLLogDetails]';
    DECLARE @DestinationTableName SYSNAME = '[DBLoanguardHistory].[dbo].[MerchantURLLogDetails]';
    DECLARE @filter NVARCHAR(MAX) ='ResponseOn  < DATEADD(day, -10, CAST(GETDATE() AS date))'; 
...
;WITH CTE_ToDelete AS (
    SELECT TOP (' + CAST(@BatchSize AS NVARCHAR(10)) + N') 
       [LogId],[ClientId],[ApplicationNo],[LoanguardId],[DocValMessageId],[Response],[URLMapName],[ResponseOn],[RequestParameters]
