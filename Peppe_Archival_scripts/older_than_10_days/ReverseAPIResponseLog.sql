...
Create or alter   PROCEDURE [dbo].[ReverseAPIResponseLog]
AS
BEGIN
    SET NOCOUNT ON;
    ...
    DECLARE @SourceTableName SYSNAME = '[DBLoanguard].[dbo].[ReverseAPIResponseLog]';
    DECLARE @DestinationTableName SYSNAME = '[DBLoanguardHistory].[dbo].[ReverseAPIResponseLog]';
    DECLARE @filter NVARCHAR(MAX) ='CreatedOn  < DATEADD(day, -10, CAST(GETDATE() AS date))'; 
...
;WITH CTE_ToDelete AS (
    SELECT TOP (' + CAST(@BatchSize AS NVARCHAR(10)) + N') 
      [APIResponseLogId],[ApplicationNo],[RequestJson],[ResponseJson],[CreatedBy],[CreatedOn],[ClientId],[ResponseErrorDetails]
