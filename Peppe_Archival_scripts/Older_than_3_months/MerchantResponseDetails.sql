...
Create or alter   PROCEDURE [dbo].[MerchantResponseDetails]
AS
BEGIN
    SET NOCOUNT ON;
    ...
    DECLARE @SourceTableName SYSNAME = ' [DBLoanguard].[dbo].[MerchantResponseDetails]';
    DECLARE @DestinationTableName SYSNAME = '[DBLoanguardHistory].[dbo].[BKPMerchantResponseDetails]';
    
	DECLARE @filter NVARCHAR(MAX) ='ResponseOn < DATEADD(MONTH, -3, CAST(GETDATE() AS DATE))'; 
	DECLARE @column NVARCHAR(MAX) ='ResponseOn';
...
;WITH CTE_ToDelete AS (
	...
	[RespDetailsId], [DocValMessageId], [Response], [ResponseOn], [URLMapName], [ClientId], [ApplicationNo], 
[iKYCUser], [RequestParameters], [Source]
