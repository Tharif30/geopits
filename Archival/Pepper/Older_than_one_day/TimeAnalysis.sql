...
Create or alter   PROCEDURE [dbo].[TimeAnalysis]
AS
BEGIN
    SET NOCOUNT ON;
    ...
    DECLARE @SourceTableName SYSNAME = '[DBLoanguard].[dbo].[TimeAnalysis]';
    DECLARE @DestinationTableName SYSNAME = '[DBLoanguardHistory].[dbo].[TimeAnalysis]';
    DECLARE @filter NVARCHAR(MAX) ='ActivityTime < CAST(GETDATE() AS DATE)'; 
	DECLARE @column NVARCHAR(MAX) ='ActivityTime';
...
;WITH CTE_ToDelete AS (
    SELECT TOP (' + CAST(@BatchSize AS NVARCHAR(10)) + N') 
      [TID],[ActivityTime],[ApplicationNo],[LoanguardId],[DocumentId],[DocumentIndex],
	  [UserId],[DStatus],[ApplicantId],[ApplicationId],[DocValMessageId]
