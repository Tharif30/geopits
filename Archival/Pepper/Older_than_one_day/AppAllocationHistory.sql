...
Create or alter   PROCEDURE [dbo].[AppAllocationHistory]
AS
BEGIN
    SET NOCOUNT ON;
    ...
    DECLARE @SourceTableName SYSNAME = '[DBLoanguard].[dbo].[AppAllocationHistory]';
    DECLARE @DestinationTableName SYSNAME = '[DBLoanguardHistory].[dbo].[AppAllocationHistory]';
    DECLARE @filter NVARCHAR(MAX) ='createdOn < CAST(GETDATE() AS DATE)'; 
...
;WITH CTE_ToDelete AS (
    SELECT TOP (' + CAST(@BatchSize AS NVARCHAR(10)) + N') 
      [AppHID],[ApplicationNo],[UserId],[HAssignStatus],[AllocationType],[CreatedBy],[CreatedOn],[CLientId],[AssignModule]
...
OUTPUT 
 ...
 Deleted.[UserId],
 Deleted.[HAssignStatus],
 Deleted.[AllocationType],
 Deleted.[CreatedBy],
 Deleted.[CreatedOn],
 Deleted.[CLientId],
 Deleted.[AssignModule]
