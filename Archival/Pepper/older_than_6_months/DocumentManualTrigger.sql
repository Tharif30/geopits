USE [DBADB]
GO
/****** Object:  StoredProcedure [dbo].[DocumentManualTrigger]    Script Date: 29-08-2025 14:49:38 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER     PROCEDURE [dbo].[DocumentManualTrigger]
AS
BEGIN
    SET NOCOUNT ON;
    ...
    DECLARE @SourceTableName SYSNAME = '[DBLoanguard].[dbo].[DocumentManualTrigger]';
    DECLARE @DestinationTableName SYSNAME = '[DBLoanguard].[dbo].[BKPDocumentManualTrigger]';
    
	DECLARE @filter NVARCHAR(MAX) =  'EXISTS (
        SELECT 1 
        FROM [DBLoanguard].[dbo].[DocumentValidatingMessages] V
        INNER JOIN [DBLoanguard].[dbo].[Applications] A ON V.ApplicationId = A.ApplicationId
        WHERE V.DocValMessageId = [DBLoanguard].[dbo].[DocumentManualTrigger].DocValMessageId
...
;WITH CTE_ToDelete AS (
    ...
    FROM (
        SELECT TOP (' + CAST(@BatchSize AS NVARCHAR(10)) + N') 
            [DocTriggerId],[DocValMessageId],[ManualTrigger],[CreatedBy],
            [CreatedOn],[ModifiedBy],[ModifiedOn],[Severity]
...
OUTPUT 
    Deleted.[DocTriggerId],
    Deleted.[DocValMessageId],
    Deleted.[ManualTrigger],
    Deleted.[CreatedBy],
    Deleted.[CreatedOn],
    Deleted.[ModifiedBy],
    Deleted.[ModifiedOn],
