...
Create or alter   PROCEDURE [dbo].[LGLog]
AS
BEGIN
    SET NOCOUNT ON;
    ...
    DECLARE @SourceTableName SYSNAME = '[DBLoanguard].[dbo].[LGLog]';
    DECLARE @DestinationTableName SYSNAME = '[DBLoanguardHistory].[dbo].[LGLog]';
...
;WITH CTE_ToDelete AS (
    SELECT TOP (' + CAST(@BatchSize AS NVARCHAR(10)) + N') 
        [Id],[LogDate],[Thread],[Logger],[Level],[Message],[Exception],[UserId],[ApplicationNo],
		[LoanguardId],[Origin],[DocumentId],[DocumentIndex],[IpAddress]
