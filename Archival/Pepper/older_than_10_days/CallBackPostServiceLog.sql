...
Create or alter   PROCEDURE [dbo].[CallBackPostServiceLog]
AS
BEGIN
    SET NOCOUNT ON;
    ...
    DECLARE @SourceTableName SYSNAME = '[DBLoanguard].[dbo].[CallBackPostServiceLog]';
    DECLARE @DestinationTableName SYSNAME = '[DBLoanguardHistory].[dbo].[CallBackPostServiceLog]';
...
;WITH CTE_ToDelete AS (
    SELECT TOP (' + CAST(@BatchSize AS NVARCHAR(10)) + N') 
        [LogId],[LogDate],[Thread],[Level],[Logger],[Message],[Exception],[ApplicationNo],[ClientId],[ProductId]
